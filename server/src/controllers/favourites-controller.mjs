...
export const addFileAsFavourite = (req, res) => {
  const query =
    'UPDATE files SET isFavourite = "Yes" WHERE fileName = ? AND userId = ?';
  ...
  db.run(query, [req.params.filename, req.user.id], (err) => {
    if (err) {
      ...
      console.log(
        `File ${req.params.filename} was successfully added to favourites.`
      );
    }
  });
};
...
export const removeFileAsFavourite = (req, res) => {
  const query =
    'UPDATE files SET isFavourite = "No" WHERE fileName = ? AND userId = ?';
  ...
  db.get(query, [req.params.filename, req.user.id], (err) => {
    if (err) {
      ...
      console.log(
        `File ${req.params.filename} was successfully removed from favourites.`
      );
    }
  });
};
...
export const displayFavourites = (req, res) => {
  const fetchFavourites =
    'SELECT f.fileName, f.folderName, f.uuid FROM files AS f WHERE f.userId = ? AND f.isFavourite = ?';
