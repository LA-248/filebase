...
export const addAsFavourite = (table, column) => async (req, res) => {
  try {
    ...
    await File.updateFavouritedStatus(table, column, favourited, itemName, userId);
...
export const removeAsFavourite = (table, column) => async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: `${itemName} was successfully removed from favourites.`,
    });
  } catch (error) {
...
export const displayFavourites = async (req, res) => {
  try {
    ...
    const files = await File.fetchFavouritedFiles(userId, isFavourite, deleted);
    const folders = await Folder.fetchFavouritedFolders(userId, isFavourite, deleted);
