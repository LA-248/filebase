...
function storeFileInformation(userId, folderName, fileName, fileSize, isFavourite, fileData) {
  const query = 'INSERT INTO files (userId, folderName, fileName, fileSize, isFavourite, fileData) VALUES (?, ?, ?, ?, ?, ?)';

  db.run(query, [userId, folderName, fileName, fileSize, isFavourite, fileData], err => {
    if (err) {
